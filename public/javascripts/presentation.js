$(document).ready( function() {
    ...
    function renderPage() {
        var curPage = $('[ng-controller="presentationCtrl"]').scope().currentPage;
        var slides = $('[ng-controller="presentationCtrl"]').scope().slides;
        var filePath = "/" + slides[curPage-1].filePath;

        console.log(filePath);

        PDFJS.getDocument(filePath).then(function (pdf) {
            if (curPage == $('[ng-controller="presentationCtrl"]').scope().currentPage) {
